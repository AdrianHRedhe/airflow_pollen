...
from airflow_pollen.common.connections import postgres, telegram

logger = logging.getLogger(__name__)


@task
def send_telegram_message_task(new_events):
    if not new_events:
        logger.info("no info to send to telegram. Ending task")
        return
    event = new_events[0]
    ...
    logger.info(f"info sent to telegram: {message}")
    return


@task
def store_in_db_task(data):
    engine = postgres.engine
    execute_query = postgres.execute_query
...
@task
def create_scd2_view_task(data):
    engine = postgres.engine
    execute_query = postgres.execute_query
    ...
    logger.info("Refreshed scd2_events view.")
    return data


@task
def find_new_scd2_events_task(data):
    engine = postgres.engine
    fetch_query = postgres.fetch_query
    ...
    if not new_events:
        logger.info("No new SCD2 events starting today.")
        return

    logger.info("New scd2 event starting today")
    return new_events
